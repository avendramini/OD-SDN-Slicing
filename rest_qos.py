...
class QoS(object):
    ...
    def _set_qos(self, cookie, rest, waiters, vlan_id):
        ...
        print("Match:", match)
        # ... (match e priority sono corretti) ...
        ...
        if action is not None:
            print("Processing actions:", action)
                # --- INIZIO PARTE MANCANTE ---
                # Ripristina la logica originale per leggere 'queue', 'mark', etc.
            if REST_ACTION_QUEUE in action:
                actions.append({'type': 'SET_QUEUE',
                    'queue_id': int(action[REST_ACTION_QUEUE])})
        ...
        # La tua logica per l'output (che è corretta)
            if 'output' in action:
                output_port = int(action['output'])
                output_action = {'type': 'OUTPUT', 'port': output_port}

        else: # Questo 'else' viene eseguito solo se l'oggetto "actions" è assente
            actions.append({'type': 'SET_QUEUE', 'queue_id': 0})

        # Aggiunge l'azione di output se presente, altrimenti GOTO_TABLE
        if output_action:
    ...
    def to_rest(openflow):
        ...
        for key, value in of_match.items():
            ...
            if key in Match._CONVERT:
                ...
                conv = dict((x, key) for key, x in conv.items())
                if value in conv:
                    match.setdefault(key, conv[value])
                else:
                    # Handle unknown values by using the raw value
                    match.setdefault(key, value)
            else:
                match.setdefault(key, value)
